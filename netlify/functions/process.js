...
5. 文件節錄中標記「【OCR 第N頁表格內容】」的段落，是圖說裡「面積計算表」等表格用 OCR
   辨識出來的文字，版面可能跑掉、數字可能黏在一起，請盡量從中判斷樓地板面積與樓層數。
6. 若面積計算表列出多棟建築物/多個欄位（例如「本工程」、「他棟建築」、「既有建築物」），
   只加總「屬於本次招標範圍」的欄位（通常是本工程／新建部分，排除既有/既存建築物），
   地上樓層取各在範圍內棟別的最大值、地下樓層同理；不要只取第一欄就當作全部。`;
